...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ sessions }) => {
  ...
  const qualifiedSessions = sessions.filter(s => s.qualificationScore >= 50);
  const completedSessions = sessions.filter(s => s.sessionStatus === 'completed');
  const highQualitySessions = sessions.filter(s => s.qualificationScore >= 70);
  const sessionsWithEmail = sessions.filter(s => s.userResponses.email);
  const averageScore = sessions.length > 0 ? Math.round(sessions.reduce((sum, s) => sum + s.qualificationScore, 0) / sessions.length) : 0;
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              ...
              {sessions.length === 0 && (
                <div className="text-center text-gray-500 py-8">
                  No sessions yet. Start a conversation to see data here.
                </div>
              )}
            </div>
          </CardContent>
